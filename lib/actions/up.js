...
module.exports = async (db, client) => {
  ...
  const { shouldMigrate, changelogCollectionName } = await config.read();

  if (typeof shouldMigrate === "function" && !shouldMigrate()) {
    throw new Error('migration up not allowed by config.shouldMigrate')
  }
